...
export(bool) var editable_left = true
export(bool) var editable_right = true
...
func refresh_hover():
	if has_connection_left:
		...
		(sibling_left != null && (sibling_left.selected || sibling_left.highlight))
	if has_connection_right:
		$Control/DotRight.visible = highlight || selected || \
		(sibling_right != null && (sibling_right.selected || sibling_right.highlight))
...
func _ready():
	refresh_hover()
	if !editable_left:
		$Control/DotLeft/ButtonLeft.disabled = true
		$Control/DotLeft/ButtonLeft.focus_mode = FOCUS_NONE
		$Control/DotLeft.modulate.a = 0.2
	if !editable_right:
		$Control/DotRight/ButtonRight.disabled = true
		$Control/DotRight/ButtonRight.focus_mode = FOCUS_NONE
		$Control/DotRight.modulate.a = 0.2

func _draw():
	# draw splines
	if a_sibling_is_pressed == null && (highlight || selected):
		if has_connection_left && (editing_left || sibling_left != null):
			Global.splinecanvas.add_spline([$PathLeft.curve, self, selected && editable_left, editing_left])
		if has_connection_right && (editing_right || sibling_right != null):
			Global.splinecanvas.add_spline([$PathRight.curve, self, selected && editable_right, editing_right])
