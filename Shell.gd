	...
	if task.semaphore.has("requires"):
		for t in queue_for_execute:
			if t.semaphore.name == task.semaphore.requires:
				return true
	return false
func run_sync(cmd, printcmd = true, printout = false):
	if printcmd:
		Console.out('sh# ' + cmd, Color(0.5,1,1,0.5))
	else:
		print('sh# \"', cmd, '\"')
	cmd = cmd.split(" ")
	var bin = cmd[0]
	...
	if printout && output.size() > 0 && output[0].length() > 0:
		...
		for line in txt.split("\n"):
			Console.out("    " + line)
	return output
func run(cmd, node, callback, printcmd = true, printout = false, semaphore = {}):
...
func _threaded_execute(args):
	if args.printcmd:
		Console.out('sh# ' + args.cmd + '', Color(0.5,1,1,0.5))
	else:
		print('sh# \"', args.cmd, '\"')
